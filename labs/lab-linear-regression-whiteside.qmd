...
whiteside |> 
  test_norm_num()  |>
  gt::tab_caption("Normality tests for Whiteside dataset")
```
...
whiteside |> 
  ...
  tab_caption("Other normality tests for Whiteside dataset")
```
...
We may further explore this question using linear modeling. The output of function `lm()` is an 
object of class `lm` (a list with attributes). This object can be fed to `broom::glance()` 
to obtain 
...
`diag_plots` is an instance of an [`S4`  class](https://adv-r.hadley.nz/s4.html). Check this with `sloop::otype(diag_plots)`.  
...
## [`S3`  classes in `R`](https://adv-r.hadley.nz/s3.html)
...
`methods(autoplot)` lists the `S3` classes for which an  autoplot method is defined.  Some methods are defined in `ggplot2`, others like `autoplot.lm` are defined in extension 
...
The output of `autoplot.lm` is an instance  of [`S4` class](https://adv-r.hadley.nz/s4.html).

See also [New trends](https://rconsortium.github.io/S7/): 
...
We also used tibbles with list  columns, that is nested dataframes. Nested dataframes show up in every implementation of dataframes (from `R` to Spark),
see: [Tidyr: article on nesting](https://tidyr.tidyverse.org/articles/nest.html)
