...
1. This call is dispatched to method `autoplot.prcomp` defined in package `ggfortify`. Look at the code using `getS3method("autoplot", "prcomp")`
2. `coord_fixed()` is useful here, since scaling axes matter here.   
...
1. Define a pipeline for building a screeplot (this works with `%>%` not with `|>`)
...
```{r}
#| label: correlation-circle-scaled-or-not
...
1. `loadings= TRUE` overlays the correlation circle and the principal components scatterplot. 
2. Helpful  if not too much overplotting. Otherwise, use hovering. 

:::

::: {.callout-tip title="solution"} 

Using translation to `plotly` enables hovering.
...
Home made biplot using `broom::augment()`
