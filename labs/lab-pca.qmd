...
`pco` is a list with 5 members. It as a class attribute `pco`. It is an object of class `prcomp` (function `prcomp()` acts as a constructor for class `pco` just as `lm()` acts as  a constructor for class `lm`). [Class `pco` is an `S3` class](https://adv-r.hadley.nz/s3.html)
...
$$\texttt{pco\$sdev[i]}^2 = \frac{\texttt{svd_Y\$d[i]}^2}{n-1}$$
