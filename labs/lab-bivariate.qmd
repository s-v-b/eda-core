...
categories: [Bivariate analysis, Boxplots, Pairplots, Mosaicplots, Correlations, Chi-square]
date: "`r Sys.time()`"
resources: 
...
tb2 <- df |>
  dplyr::count(CATEGORIE, SEXE)

tb2  |>
  gt::gt() |>
  gt::tab_caption("Contingency table in long form (full)")
```
...
tb2 |> 
  tidyr::pivot_wider(id_cols=CATEGORIE, 
              names_from=SEXE, 
              values_from=n) |>
  gt::gt() |>
  gt::tab_caption("2-ways contingency table in wide form (full)")
...
mosaicplot(~ CATEGORIE + SEXE, 
           tb, 
           main="Données Recensement, ~ CATEGORIE + SEXE", 
           las=2)
```

```{r}
#| label: mosaics-2
mosaicplot(~ SEXE + CATEGORIE, 
           tb, 
           main="Données Recensement, ~ SEXE + CATEGORIE",
           las=2)


```

Mosaic plots belong to the family of area plots. For each couple of levels, a rectangle is plotted with surface area proportional to the frequency of the couple of levels. 

In a 2-way mosaicplots, the width of the rectangles is proportional to the marginal count of the levels of the variable mapped to the `X` axis (first variable in the formula). For each level says `a` of the `X` variable, the height of the subrectangle corresponding to a level say `b` of the `Y` variable  is proportional to the ratio between the frequency if couple `(a,b)` and the marginal frequency of `a`.

The mosaicplot with formula `~ CATEGORIE + SEXE` allows us to visualize the frequencies of the different levels of `CATEGORIE`, and for each level of `CATEGORIE` the sex ratio of that level. 
...
while complying with the `ggplot2` look and feel.
...
The next layer will be used repeatedly. 

```{r}
#| label: rotation-label-trick
#| warning: true
#| eval: true
rot_x_text <- ggplot2::theme(
...
df |>
  ...
  ggmosaic::geom_mosaic(
    ggplot2::aes(
      x=ggmosaic::product(SEXE, CATEGORIE), 
     fill=SEXE)) +
...
rare_categories |>
  gt::gt() |>
  gt::tab_caption("Rare categories (n < 40)")
```
...
df |>
  ...
  gt::gt() |>
  gt::tab_caption("Contingency table in long form") 
```
...
gt_o <- df |> 
  ...
  tidyr::pivot_wider(
    ...
    values_from=n) |>
  gt::gt() |>
  gt::tab_caption("Contingency table in wide form\\n (2-way table)") 
...
```{r}
mosaicplot(~ CATEGORIE + SEXE, df, las=2)
```
...
Assuming that the data are collected using i.i.d. sampling, we may test 

- Null hypothesis: the population correlation coefficient is zero
- Alternative: the population correlation coefficient differs from zero

This binary testing problem (where both the Null and the Alternative are composite) can be stated for the three flavors of correlation coefficients described above. 

Note that we have stated the problem with two-sided alternative. It can also be stated with one-sided alternatives ()

::: {.callout-note title="Question"}

Use function `cor.test()` to test correlation between `AGE` and `SAL_HOR`
...
foo <- function(lst) {
  ...
  bind_rows() |>
  gt::gt() |>
  gt::fmt_number(c(estimate, conf.low, conf.high), decimals=3) |>
  gt::fmt_scientific(c(statistic, p.value), decimals=2) |>
  gt::tab_caption("Correlation tests for AGE and SAL_HOR")
}

methods |>
  purrr::map( \(x) {cor.test(x=df$AGE, y=df$SAL_HOR, method=x, exact=F)})|>
  foo() 
...
```{r}
methods |>
  purrr::map( \(x) {cor.test(x=df$AGE, y=df$SAL_HOR, alternative="greater", method=x, exact=F)})|>
  foo()  
