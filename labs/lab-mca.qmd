...
In the panel questionnaire, some questions have `race` in the field `id`. 
...
A number of questions are related to the *confidence* topic
...
## US field of ideological dispositions
...
res.mca |> 
  augment_mca(confidence_data) |> 
  ggplot() +
  aes(x=`Dim 3`, y=`Dim 4`, fill=consci) +
...
```{r}
(
  tidy_mca_var(res.mca) |>  
  ggplot() +
    aes(x=`Dim 1`, y=`Dim 2`, label=levels) +
    geom_point() +
    coord_fixed()
) |>
   plotly::ggplotly()
...
(
  tidy_mca_var(res.mca) |>  
  ggplot() +
    aes(x=`Dim 3`, y=`Dim 4`, label=levels) +
    geom_point() +
    coord_fixed()
) |>
   plotly::ggplotly()
